...
const
  ...
  containers = {
    ...
    afastamentos: [
      //'afastamentoPeriodoInicio',
      //'afastamentoPeriodoFim',
      //'afastamentoNumProcesso',
      "afastamentoPeriodo",
  ...
  nomesAmigaveis = {
    ...
    acoesAcaoCadastrada: "Cadastro no SIGPROJ?",
    acoesSIGPROJ: "Número SIGPROJ",
...
const acoesDinamicas = () => {
  const nomeInput = "acoes",
    ...
    campoDinamico = num => `      
      ...
      <div class='numero-sigproj field' id='numero-sigproj-${num}'>
        <input type='hidden' name='${nomeInput}SIGPROJ' value = '-'>
      </div> 
  ...
  const listeners = [
    {
      elemento: `select[name=${nomeInput}AcaoCadastrada]`,
      evento: "change",
      funcao: (e, num) => {
        let html;
        if (e.target.value === "sim") {
          html = `
            <label for='acao-sigproj-${num}'>
              <p>Número SIGPROJ:</p>            
            </label>
            <input type='text' id="acao-sigproj-${num}" name="${nomeInput}SIGPROJ" data-section='4' data-subsection='user-acoes' ${planid
            .emEdicao.homologado && 'readonly tabindex="-1"'} required >`;
        } else {
          html = `<input type='hidden' name='${nomeInput}SIGPROJ' value = '-' data-subsection='user-acoes'>`;
        }
        document.getElementById(`numero-sigproj-${num}`).innerHTML = html;
      }
    },
    {
      ...
      if (plSelecionado[containerArray[i][1][j]].length) {
        ...
        for (let k = 0; k < valores.length; k++) {
          if (propriedade === "orientacoesInstituicao" && valores[k] !== "ufrj") {
            ...
            `;
          } else if (propriedade === "acoesAcaoCadastrada" && valores[k] === "sim") {
            getSelectedOption(
              document.querySelectorAll(`#acao-cadastrada-${valores.length - 1 - k} option`),
              valores[k]
            );
            document.getElementById(`numero-sigproj-${valores.length - 1 - k}`).innerHTML = `
            <label for='acao-sigproj-${valores.length - 1 - k}'>
              <p>Número SIGPROJ:</p>            
            </label>
            <input type='text' id="acao-sigproj-${valores.length -
              1 -
              k}" name="acoesSIGPROJ" data-section='4' data-subsection='user-acoes' value='${
              plSelecionado.acoesSIGPROJ[k]
            }' ${planid.emEdicao.homologado && 'readonly tabindex="-1"'} required >  
            `;
          } else {     
            document.querySelectorAll(`[name=${propriedade}]`)[k].value = valores[k];
          }
          if (propriedade === "disciplinaNivel" && valores[k] === "especializacao") {
            const campoCodigo = document
              .querySelectorAll(`[name=${propriedade}]`)
              [k].parentElement.parentElement.querySelector(".campo-codigo-disciplina");
